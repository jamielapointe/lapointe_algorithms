...
TEST(TestCountSplitInversions, BasicTest00) {
  static constexpr std::size_t   arr_size{6};
  std::array<uint32_t, arr_size> base_array{1, 3, 5, 2, 4, 6};
  static constexpr uint32_t      truth_split_inversion_count{3};

  uint32_t count = LaPointe_Algorithms::Data_Structures::Count_Split_Inversions::sort_count_inv(base_array.begin(),
                                                                                                base_array.end());
  ASSERT_EQ(count, truth_split_inversion_count);
}

TEST(TestCountSplitInversions, CourseraQuiz) {
  ...
  uint32_t                  count = LaPointe_Algorithms::Data_Structures::Count_Split_Inversions::sort_count_inv(
                       integers.begin(), integers.end(), dest_vector->begin(), dest_vector->end());
  ASSERT_EQ(count, truth_split_inversion_count);
}
